...
ManagedPointer pack_image_bitmax(unsigned char *rgb_data, int channels) {
  uint16_t *data = static_cast<uint16_t *>(
      calloc(IMAGE_EDGE_PX * IMAGE_EDGE_PX, PIXEL_DEPTH_BYTES));
  // If a channel is over 50% a colour, it's on.
...
ManagedPointer pack_image_565(unsigned char *rgb_data, int channels) {
  ...
  for (int i = 0; i < IMAGE_EDGE_PX * IMAGE_EDGE_PX; i++) {
    ...
    // Pack it into the output data
    data[i * PIXEL_DEPTH_BYTES] = ((r & 0b11111000) << 3) | (g >> 5);
    data[i * PIXEL_DEPTH_BYTES + 1] =
        ((g & 0b00011100) << 3) | ((b & 0b11111000) >> 3);
  }

  return ManagedPointer(data, std::free);
}
...
int main(int argc, char **argv) {
  ...
  for (int img_idx = 2; img_idx < argc; img_idx++) {
    ...
    ManagedPointer packed = pack_image_bitmax(img.get(), channels);
